...
<template lang="pug">
  div.settings-tab
    div.settings-form
      label
        div.setting-label
          span {sharedProps.localize("PricingFactor")}
          strong {sharedProps.pricingFactor}%
        input(type="range" value="{sharedProps.pricingFactor}" min="50" max="200" step="1" on:input!="{(e) => sharedProps.onPricingFactorChange?.(e.target.value)}")
        div.setting-range
          span 50%
          span 200%
        p.setting-help (affects all prices)
      label
        div.setting-label
