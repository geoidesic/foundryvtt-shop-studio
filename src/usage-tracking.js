/**
 * Usage Tracking Integration for Shop Studio
 * 
...
class UsageTracker {
  constructor() {
    this.moduleKey = 'shop_studio';
    this.moduleName = 'Shop Studio';
    this.moduleVersion = pkg.version || '0.0.1';
   ...
   */
  getUserType() {
    const roles = ['creator', 'gm', 'player'];
    return roles[game.user?.role ?? -1] ?? 'unknown';
  }
}

// Export a singleton instance and expose on window for external access
